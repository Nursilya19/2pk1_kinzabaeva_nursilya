...
namespace PZ_16
{
    internal class Program
    {
        ...
        static byte enemyCount = 10; // Количество врагов
        ...
        static void Victory()
        {
            ...
            Console.Clear();
            Centertext($"Вы победили врагов. Пройдено {stepCount} шагов", centerY);
            Centertext("Нажмите Enter для выхода из игры", centerY + 1);
        ...
        static void BuffUp() // Логика усилиления
        {
        ...
        static void Fight() // Логика обмена ударами
        {
            if (map[playerX, playerY] == 'E') // Если встать на врага
            {
                while (playerHP > 0 && enemyHP > 0) // Пока оба живы
                {
                    ...
                    if (playerHP <= 0) // Если здоровье игрока кончилось, экран проигрыша
                    {
                        ...
                        string[] texts = {$"Вы проиграли, пройдено {stepCount} шагов.", "Нажмите Enter для возвращения в меню"};
